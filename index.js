...
const runOnPages = async (fn, { limit = 1, skip = 0 } = {}) => {
  const params = { limit, skip, props: 'id,slug,title,thumbnail,metadata.headline,metadata.tags,published_at,content' }
...
const run = async () => {
  ...
  await fsExtra.copy('static/robots.txt', 'dist/robots.txt')
